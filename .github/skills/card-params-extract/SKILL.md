...
- `params.json` のうち、人が読む説明文（`taxonomy.*`, `caption`, `flavor_quote`, `entries.*`, `moves.*.description`, `affinity.*.note`）は、**小学生でも意味を想像しやすい言葉**を優先する
- 抽象的すぎる造語、社内っぽい言い回し、哲学っぽい表現は避ける。難しい言葉を使いたいときは、必ず身近なたとえや具体物に言い換える
- ユーモアは歓迎。ただし「ちょっと笑える観察メモ」くらいにとどめ、本人を茶化したり、きつい比喩にはしない
- 造語を強めに使ってよいのは `primary_name` / `alias` / `moves.hidden.name` まで。それ以外の本文は、読んだ瞬間に情景が浮かぶ普通の言葉を優先する
- 迷ったら「図鑑を読む小学生が、すぐに頭の中で絵にできるか」で判断する
...
```json
{
  ...
  "creature": {
    ...
    "taxonomy": {
      ...
      "genus": "直感属",
      "species": "起爆種",
      "sentence": "突破類・直感属・起爆種。思いついたらまず前に出て、場の空気をぱっと動かすタイプとして分類される。"
    },
    "types": ["創発", "行動"],
    "rarity": 4,
    "stats": {
...
- 耐性: §2「PoC / ハッカソン / 突発障害」 → **「急なお願い」「ふわっと相談」**
...
- ユーモアは 1 セクションに 1 か所程度まで。読みやすさを壊すほど盛らない
- 読み手が止まりやすい漢語の連続（例: 抽象化、創発、共鳴構造など）は避け、必要なら「〜するのが得意」「〜をまとめるのがうまい」と言い換える
